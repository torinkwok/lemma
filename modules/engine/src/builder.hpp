...
public:
    ...
    void run() override
    {
        ...
        // (n-1)*(n)/2 times, where n=HOLDEM_MAX_DECK.
        for (uint8_t i = 0; i < HOLDEM_MAX_DECK - 1; i++) {
            for (uint8_t j = i + 1; j < HOLDEM_MAX_DECK; j++) {
                std::string cards;
...
public:
    ...
    void cluster_private(std::string pub_cluster_file, std::string feature_file)
    {
        ...
        for (auto [bucket, board_indicies]: pub_bucket_map) {
            logger::debug("finding priv+pub belonging to bucket %d (%d public canon boards)", bucket,
                          board_indicies.size());
    ...
    void cluster_public()
    {
        ...
        for (int t = 0; t < max_iterr_; t++) {
            ...
            for (int i = 0; i < pub_states; i++) {
                ...
                int closest_cluster =
                        std::distance(clusterDistances,
                                      std::min_element(clusterDistances, clusterDistances + num_clusters_));
