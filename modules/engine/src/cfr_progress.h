#ifndef AUTODIDACT_MODULES_ENGINE_SRC_CFR_STATE_H_
#define AUTODIDACT_MODULES_ENGINE_SRC_CFR_STATE_H_
...
struct sCFRProgress
{
    ...
    bool operator<(const sCFRProgress &that) const
    {
        ...
        if (!(time_flag && iter_flag && expt_flag && std_flag)) {
            ...
            if (!iter_flag) {
                logger::info("cfr terminated || iter_flag ended at iteration = %d", this->iteration);
            }
            if (!expt_flag) {
...
#endif //AUTODIDACT_MODULES_ENGINE_SRC_CFR_STATE_H_
